...
func TestUpdateMaxFees(t *testing.T) {
	...
	if ok := api.RefreshFees(ctxTimeout()); !ok {
...
func TestAPI_GetFee(t *testing.T) {
	...
	_, _, err = account.GetNames(ctxTimeout(), api)
	...
	actual, err := api.GetFee(ctxTimeout(), account.Addresses[0].FioAddress, FeeAddPubAddress)
	if actual != 0 {
...
func Test_FeeOverUnderflow(t *testing.T) {
	...
	_, _, err = api.SignTransaction(ctxTimeout(), NewTransaction([]*Action{goodFee}, opts), opts.ChainID, CompressionNone)
	if err != nil {
...
func Test_NewSubmitMultiplier(t *testing.T) {
	...
	_, err = api.SignPushActions(ctxTimeout(), NewSetFeeMult(multiplier, acc.Actor))
	if err != nil {
		t.Error(err)
	}
