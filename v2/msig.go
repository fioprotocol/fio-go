...
func (api *API) GetApprovals(ctx context.Context, scope Name, limit int) (more bool, info []*MsigApprovalsInfo, err error) {
	name, err := eos.StringToName(string(scope))
	if err != nil {
		return false, nil, err
	}
	res, err := api.GetTableRows(ctx, eos.GetTableRowsRequest{
		JSON:  true,
...
func (api *API) NewSignedMsigPropose(ctx context.Context, proposalName Name, approvers []string, actions []*Action, expires time.Duration, signer *Account, txOpt *TxOptions) (*eos.PackedTransaction, error) {
	if len(actions) == 0 {
	...
	propTxSigned, propTxPacked, err := api.SignTransaction(ctx, propTx, txOpt.ChainID, CompressionNone)
	if err != nil {
		return nil, err
	}
	...
	_, packedTx, err := api.SignTransaction(ctx, newTx, txOpt.ChainID, CompressionZlib)
	if err != nil {
		return nil, err
	}
...
func (api *API) GetProposalTransaction(ctx context.Context, proposalAuthor eos.AccountName, proposalName eos.Name) (*MsigProposal, error) {
...
func (api *API) GetProposals(ctx context.Context, offset int, limit int) (more bool, scopes map[string]int, err error) {
	res, err := api.GetTableByScopeMore(ctx, eos.GetTableByScopeRequest{
		Code:       "eosio.msig",
		Table:      "proposal",
