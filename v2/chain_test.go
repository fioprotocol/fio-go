...
func TestAPI_GetCurrentBlock(t *testing.T) {
	...
	b := api.GetCurrentBlock(ctxTimeout())
	if b == 0 {
...
func TestAPI_GetTableRowsOrder(t *testing.T) {
	...
	gtr, err := api.GetTableRows(ctxTimeout(), eos.GetTableRowsRequest{
		Code:       "eosio",
		Scope:      "eosio",
		Table:      "producers",
		LowerBound: "2",
	...
	gtro, err := api.GetTableRowsOrder(ctxTimeout(), GetTableRowsOrderRequest{
		Code:       "eosio",
		Scope:      "eosio",
		Table:      "producers",
		LowerBound: "2",
...
func TestAPI_GetBlockHeaderState(t *testing.T) {
	...
	info, err := api.GetInfo(ctxTimeout())
	if err != nil {
		t.Error(err)
		return
	}
	bhs, err := api.GetBlockHeaderState(ctxTimeout(), info.HeadBlockNum)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestAPI_PushEndpointRaw(t *testing.T) {
	...
	_, tx, err := api.SignTransaction(ctxTimeout(),
		NewTransaction(
			[]*Action{
	...
	_, err = api.PushEndpointRaw(ctxTimeout(), "/v1/chain/transfer_tokens_pub_key", tx)
	if err != nil {
		t.Error(err)
	}
}
...
func TestAPI_GetRefBlock(t *testing.T) {
	...
	_, _, err = api.GetRefBlock(ctxTimeout())
	if err != nil {
		t.Error(err)
	}
}
...
func TestAPI_GetTableByScopeMore(t *testing.T) {
	...
	//res, err := api.GetTableByScopeMore(eos.GetTableByScopeRequest{ // FIXME
	_, err = api.GetTableByScopeMore(ctxTimeout(), eos.GetTableByScopeRequest{
		Code:  "eosio",
		Table: "producers",
		Limit: 1,
	})
