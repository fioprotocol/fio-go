...
func (api *API) GetPublic(ctx context.Context, fioAddress Address, chain string, token string) (address PubAddress, found bool, err error) {
	return api.PubAddressLookup(ctx, fioAddress, chain, token)
}
...
func (api *API) GetAllPublic(ctx context.Context, fioAddress Address) ([]TokenPubAddr, error) {
	gtr, err := api.GetTableRows(ctx, eos.GetTableRowsRequest{
		Code:       "fio.address",
		Scope:      "fio.address",
		Table:      "fionames",
...
func (api *API) PubAddressLookup(ctx context.Context, fioAddress Address, chain string, token string) (address PubAddress, found bool, err error) {
	if token == "" {
		token = chain
	}
	if !fioAddress.Valid() {
	...
	err = api.call(ctx, "chain", "get_pub_address", &pubAddressRequest{
		FioAddress: string(fioAddress),
		TokenCode:  token,
		ChainCode:  chain,
	}, &address)
...
func (api *API) GetFioNames(ctx context.Context, pubKey string) (names FioNames, found bool, err error) {
	err = api.call(ctx, "chain", "get_fio_names", &getFioNamesRequest{FioPublicKey:pubKey}, &names)
	if err != nil {
...
func (api *API) getFioDomainsOrNames(ctx context.Context, endpoint string, pubKey string, offset uint32, limit uint32) (domains *FioNames, err error) {
	_, err = ActorFromPub(pubKey)
	if err != nil {
		return nil, err
	}
	result := &FioNames{}
	err = api.call(ctx, "chain", endpoint, &getFioNamesRequest{
		FioPublicKey: pubKey,
		Limit:        limit,
		Offset:       offset,
	}, result)
...
func (api *API) GetFioDomains(ctx context.Context, pubKey string, offset uint32, limit uint32) (domains *FioNames, err error) {
	return api.getFioDomainsOrNames(ctx, "get_fio_domains", pubKey, offset, limit)
}
...
// which may not provide the full set of results because of (silent, without error) database query timeout issues.
// offset and limit must both be positive numbers. The returned uint32 specifies how many more results are available.
func (api *API) GetFioAddresses(ctx context.Context, pubKey string, offset uint32, limit uint32) (addresses *FioNames, err error) {
	return api.getFioDomainsOrNames(ctx, "get_fio_addresses", pubKey, offset, limit)
}
...
func (api *API) GetFioNamesForActor(ctx context.Context, actor string) (names FioNames, found bool, err error) {
	...
	return api.GetFioNames(ctx, results[0].Clientkey)
}
...
func (api *API) GetDomainOwner(ctx context.Context, domain string) (actor *eos.AccountName, err error) {
...
func (api *API) AvailCheck(ctx context.Context, addressOrDomain string) (available bool, err error) {
	isReg := &AvailCheckResp{}
	err = api.call(ctx, "chain", "avail_check", &AvailCheckReq{FioName: addressOrDomain}, isReg)
	if err != nil {
		return false, err
	}
...
func (api *API) GetBundleRemaining(ctx context.Context, a Address) (remaining int, err error) {
	if !a.Valid() {
