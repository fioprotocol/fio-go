...
func TestOBT(t *testing.T) {
	...
	apiB, _, err := NewConnection(ctxTimeout(), bob.KeyBag, api.BaseURL)
	if err != nil {
		t.Error(err)
		return
	}
	aliceAddresses, err := api.GetFioAddresses(ctxTimeout(), alice.PubKey, 0, 100)
	if err != nil {
		t.Error(err)
	}
	bobAddresses, err := api.GetFioAddresses(ctxTimeout(), bob.PubKey, 0, 100)
	if err != nil {
		t.Error(err)
	}

	// already have it, but be thorough and walk through entire process
	bobPub, found, err := api.PubAddressLookup(ctxTimeout(), Address(bobAddresses.FioAddresses[0].FioAddress), "FIO", "FIO")
	if err != nil || !found {
	...
	sent, ok, err := api.GetSentFioRequests(ctxTimeout(), alice.PubKey, 100, 0)
	if err != nil {
		t.Error(err)
		return
	}
	...
	_, err = api.SignPushActions(ctxTimeout(), cnlReq)
	if err != nil {
		t.Error(err)
	}
	time.Sleep(250 * time.Millisecond)
	// ensure it's on the list of cancelled requests
	cancelled, err := api.GetCancelledRequests(ctxTimeout(), alice.PubKey, 100, 0)
	if err != nil {
		t.Error(err)
	...
	pending, ok, err := apiB.GetPendingFioRequests(ctxTimeout(), bob.PubKey, 100, 0)
	if err != nil {
		t.Error(err)
		return
	}
	...
	for i := len(pending.Requests) - 1; i >= 0; i-- {
		if pending.Requests[i].PayeeFioPublicKey == alice.PubKey {
			...
			_, err = apiB.SignPushActions(ctxTimeout(),
	...
	afterRej, ok, err := apiB.GetPendingFioRequests(ctxTimeout(), bob.PubKey, 100, 0)
	if err != nil {
		t.Error(err)
		return
	}
