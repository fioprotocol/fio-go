...
func TestTpid(t *testing.T) {
	...
	var opts = &TxOptions{}
	api, opts, err = NewConnection(ctxTimeout(), account.KeyBag, api.BaseURL)
	if err != nil {
		t.Error(err)
		return
	}
	...
	for i := 0; i < 10; i++ {
		...
		_, err = fApi.SignPushTransaction(ctxTimeout(), NewTransaction(
	...
	_, packed, err := api.SignTransaction(ctxTimeout(), NewTransaction(
	...
	j, err := api.PushTransactionRaw(ctxTimeout(), packed)
