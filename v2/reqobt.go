...
type getCancelledRequests struct {
...
func (api *API) GetPendingFioRequests(ctx context.Context, pubKey string, limit int, offset int) (pendingRequests PendingFioRequestsResponse, hasPending bool, err error) {
	return api.getFioRequests(ctx, "pending", pubKey, limit, offset)
}

// GetSentFioRequests looks for sent requests
func (api *API) GetSentFioRequests(ctx context.Context, pubKey string, limit int, offset int) (sentRequests PendingFioRequestsResponse, hasSent bool, err error) {
	return api.getFioRequests(ctx, "sent", pubKey, limit, offset)
}

// TODO: use api.call here:
func (api *API) getFioRequests(ctx context.Context, requestType string, pubKey string, limit int, offset int) (pendingRequests PendingFioRequestsResponse, hasPending bool, err error) {
	...
	switch requestType {
	case "pending":
		req, err = http.NewRequestWithContext(ctx, "POST", api.BaseURL+`/v1/chain/get_pending_fio_requests`, bytes.NewBuffer(j))
	case "sent":
		req, err = http.NewRequestWithContext(ctx, "POST", api.BaseURL+`/v1/chain/get_sent_fio_requests`, bytes.NewBuffer(j))
	}
	if err != nil {
		return PendingFioRequestsResponse{}, false, err
	}
...
func (api *API) GetFioRequest(ctx context.Context, requestId uint64) (request *FundsReqTableResp, err error) {
	...
	if len(r) > 0 && r[0] != nil {
		...
		r, _, err = api.checkFRTRMismatch(ctx, r)
		return r[0], err
	}
	return
}
...
func (api *API) checkFRTRMismatch(ctx context.Context, req []*FundsReqTableResp) (resp []*FundsReqTableResp, ok bool, err error) {
	ok = true
	for _, r := range req {
		payerPub, _, err := api.PubAddressLookup(ctx, Address(r.PayerFioAddress), "FIO", "FIO")
		if err != nil {
		...
		payeePub, _, err := api.PubAddressLookup(ctx, Address(r.PayeeFioAddress), "FIO", "FIO")
		if err != nil {
			return req, false, err
		}
...
// existing request.
func (api *API) GetFioRequestStatus(ctx context.Context, requestId uint64) (hasResponse bool, request *FundsRequestStatusResp, err error) {
	resp, err := api.GetTableRows(ctx, eos.GetTableRowsRequest{
		Code:       "fio.reqobt",
		Scope:      "fio.reqobt",
		Table:      "fioreqstss",
