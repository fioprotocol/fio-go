...
func NewConnection(ctx context.Context, keyBag *eos.KeyBag, url string) (*API, *TxOptions, error) {
	...
	api.SetCustomGetRequiredKeys(
		func(ctx context.Context, tx *eos.Transaction) (keys []ecc.PublicKey, e error) {
...
func NewWifConnect(ctx context.Context, wif string, url string) (account *Account, api *API, opts *TxOptions, err error) {
	account, err = NewAccountFromWif(wif)
	if err != nil {
		return
	}
	api, opts, err = NewConnection(ctx, account.KeyBag, url)
	return
}
...
func (api *API) GetCurrentBlock(ctx context.Context) (blockNum uint32) {
...
func (api *API) PushEndpointRaw(ctx context.Context, endpoint string, body interface{}) (out json.RawMessage, err error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", api.BaseURL+endpoint, jsonBody)
	if err != nil {
...
func (api *API) AllABIs(ctx context.Context) (map[eos.AccountName]*eos.ABI, error) {
	type contracts struct {
		Owner string `json:"owner"`
	}
	table, err := api.GetTableRows(ctx, eos.GetTableRowsRequest{
		Code:  "eosio",
		Scope: "eosio",
		Table: "abihash",
		JSON:  true,
	...
	for _, name := range result {
		bi, err := api.GetABI(ctx, eos.AccountName(name.Owner))
		if err != nil {
			continue
		}
...
// getTableByScopeResp is used to deal with string vs bool vs int in More field:
// not entirely sure why some of the responses from FIO chain have a different type, but this handles it.
type getTableByScopeResp struct {
	More interface{}     `json:"more"`
	Rows json.RawMessage `json:"rows"`
}

// GetTableByScopeMore handles responses that have either a bool, int or a string as the more response.
func (api *API) GetTableByScopeMore(ctx context.Context, request eos.GetTableByScopeRequest) (*eos.GetTableByScopeResp, error) {
	gt := &getTableByScopeResp{}
	err := api.call(ctx, "chain", "get_table_by_scope", &request, gt)
	if err != nil {
		return nil, err
	}
	return &eos.GetTableByScopeResp{
		More: fmt.Sprint(gt.More), // just blindly convert anything returned as a string, should handle bools, numbers, etc.
		Rows: gt.Rows,
	}, nil
}
...
func (api *API) GetRefBlock(ctx context.Context) (refBlockNum uint32, refBlockPrefix uint32, err error) {
	// get current block:
	currentInfo, err := api.GetInfo(ctx)
	if err != nil {
		return 0, 0, err
	}
	// uint16: block % (2 ^ 16)
...
func (api *API) GetSupportedApis(ctx context.Context) (onlySafe bool, apis []string, err error) {
	supported := &getSupportedApisResp{}
	err = api.call(ctx, "node", "get_supported_apis", "", supported)
