...
func (api *API) GetBpJson(ctx context.Context, producer eos.AccountName) (*BpJson, error) {
	return api.getBpJson(ctx, producer, false)
}

// allows override of private ip check for tests
// TODO: respect context on http client
func (api *API) getBpJson(ctx context.Context, producer eos.AccountName, allowIp bool) (*BpJson, error) {
...
func (api *API) GetVotes(ctx context.Context, account string) (votedFor []string, err error) {
	getVote, err := api.GetTableRows(ctx, eos.GetTableRowsRequest{
		Code:  "eosio",
		Scope: "eosio",
		Table: "voters",
