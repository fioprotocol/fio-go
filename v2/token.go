...
func (api *API) GetBalance(ctx context.Context, account eos.AccountName) (float64, error) {
	a, err := api.GetCurrencyBalance(ctx, account, "FIO", eos.AccountName("fio.token"))
	if err != nil {
		return 0.0, err
	}
	if len(a) > 0 {
