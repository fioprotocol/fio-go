...
func TestAPI_HistGetBlockTxids(t *testing.T) {
	...
	api, _, err := NewConnection(ctxTimeout(), nil, "http://dev:8080")
	if err != nil {
		return
	}
	blocks, err := api.HistGetBlockTxids(ctxTimeout(), 123)
	if err != nil {
		t.Error(err)
		return
	}
	...
	trace, err := api.GetTransaction(ctxTimeout(), blocks.Ids[0].String())
	if err != nil {
		t.Error(err)
		return
	}
...
func TestApi_getMaxActions(t *testing.T) {
	...
	if !api.HasHistory(ctxTimeout()) {
		fmt.Println("history api not available, skipping GetMaxActions test")
		return
	}
	h, err := api.GetMaxActions(ctxTimeout(), "eosio")
	if err != nil {
		t.Error(err)
		return
	}
...
func TestAPI_GetActionsUniq(t *testing.T) {
	...
	traces, err := api.GetActionsUniq(ctxTimeout(), "o2ouxipw2rt4", 1000, 0)
	if err != nil {
		t.Error(err)
		return
	}
	...
	for _, trace := range traces {
		if seen[trace.Receipt.ActionDigest.String()] {
			t.Error("duplicate trace located")
			return
		}
		seen[trace.Receipt.ActionDigest.String()] = true
	}
}
