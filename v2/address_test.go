...
func TestAPI_GetFioNames(t *testing.T) {
	...
	for _, toTest := range []string{"GetFioNames", "GetFioDomains", "GetFioAddresses"} {
		...
		case "GetFioNames":
			var ok bool
			var n FioNames
			n, ok, err = api.GetFioNames(ctxTimeout(), pubkey)
			if err != nil {
			...
			hasAddress = true
		case "GetFioDomains":
			names, err = api.GetFioDomains(ctxTimeout(), pubkey, 0, 100)
			if err != nil {
				t.Error(toTest + " " + err.Error())
				break
			}
		...
		case "GetFioAddresses":
			names, err = api.GetFioAddresses(ctxTimeout(), pubkey, 0, 100)
			if err != nil {
				t.Error(toTest + " " + err.Error())
				break
			}
...
func TestAddress(t *testing.T) {
	...
	apiA, optsA, err := NewConnection(ctxTimeout(), accountA.KeyBag, api.BaseURL)
	...
	_, err = api.SignPushTransaction(ctxTimeout(), NewTransaction(
	...
	// ensure available
	ok, err := api.AvailCheck(ctxTimeout(), domain)
	if err != nil {
	...
	// register a domain
	_, err = apiA.SignPushTransaction(ctxTimeout(), NewTransaction(
	...
	// confirm owner
	o, err := api.GetDomainOwner(ctxTimeout(), domain)
	if err != nil {
		t.Error(err)
	...
	rem, err := apiA.GetBundleRemaining(ctxTimeout(), Address(names[2]+"@"+domain))
	if err != nil {
	...
	// query by actor
	fioNames, ok, err := api.GetFioNamesForActor(ctxTimeout(), string(accountA.Actor))
	if err != nil {
		t.Error(err)
	}
	if !ok {
	...
	pubAddress, ok, err := api.PubAddressLookup(ctxTimeout(), Address(names[2]+"@"+domain), "chain", "token")
	if err != nil {
		t.Error(err)
	}
	if !ok {
	...
	addrs, err := api.GetAllPublic(ctxTimeout(), Address(names[2]+"@"+domain))
	if err != nil {
	...
	pubAddress, ok, err = api.PubAddressLookup(ctxTimeout(), Address(names[2]+"@"+domain), "FIO", "FIO")
	if err != nil {
		t.Error(err)
	}
	if !ok {
	...
	// verify
	newOwner, err := api.GetDomainOwner(ctxTimeout(), domain)
	if err != nil {
		t.Error(err)
	}
...
func TestRemoveAddr(t *testing.T) {
	...
	i, _, err := account.GetNames(ctxTimeout(), api)
	if err != nil {
		t.Error(err)
		return
	}
	...
	_, err = api.SignPushActions(ctxTimeout(), naa)
	if err != nil {
		t.Error(err)
		return
	}
	...
	// ensure it was added
	if a, _, err := api.PubAddressLookup(ctxTimeout(), Address(account.Addresses[0].FioAddress), junk, junk); err != nil || a.PublicAddress == "" {
	...
	_, err = api.SignPushActions(ctxTimeout(), rm)
	if err != nil {
		t.Error(err)
		return
	}
	...
	_, err = api.SignPushActions(ctxTimeout(), rmfr)
	if err != nil {
		t.Error(err)
		return
	}
