...
func TestSignatureSerialization(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, `FIO5jSQLpKBHLaMtuzkftnYE6bCMA5Jxso8f22uZyKj6cDEp32eSj`, pubKey.String()) // not checked after..
...
func TestSignaturePublicKeyExtraction(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedPubKey: "FIO7KtnQUSGVf4vbFE2eQsWmDp4iV93jVcSmdQXtRdRRnWj2ubbFW",
		},
		{
			name:                "R1",
			signature:           "SIG_R1_KE33Ucjr5N3GR4ZosFh8KtGMytHHNtnmdUaSoMLJVXpVXoC8B9zfoXYrLiQJZqroe3LKciaP2uJT7Myqqoo4PZH7iSnso8",
...
func TestEOSIOCSigningComparison(t *testing.T) {
	...
	assert.Equal(t, "FIO6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", pubKey.String())
}
